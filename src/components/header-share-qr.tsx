...
export function HeaderShareQr() {
  ...
  // Mark client-mounted so createPortal is safe (no SSR mismatch)
  ...
  // The sheet is portalled to document.body so it fully escapes the
  // sticky header\'s stacking context. Without this, fixed + z-index
  // behaves incorrectly on iOS Safari when inside a sticky ancestor.
  const sheet =
    open && mounted
      ? createPortal(
          <div
          ...
          >
            <div
              className="relative w-full overflow-y-auto rounded-t-2xl border-t border-x border-border bg-background shadow-2xl max-h-[90vh] sm:mx-auto sm:mb-6 sm:max-w-sm sm:rounded-2xl sm:border"
                ...
                {tab === 'my-qr' && (
                  <div className="flex flex-col items-center gap-4 pb-6">
                    ...
                    <p className="text-center text-[10px] text-muted-foreground">
                      {shortAddr}&nbsp;&middot;&nbsp;Only you can decrypt
                      what&apos;s sent to you
                    </p>
                  </div>
                )}
                  ...
                  <div className="flex flex-col items-center gap-3 pb-6">
                    {!scanResult ? (
                      <>
                        ...
                        {!hasBarcodeDetector && !cameraError && (
                          <p className="text-center text-[10px] text-muted-foreground">
                            Live scanning uses the camera above. On iOS, tap
                            the button to use your camera roll instead.
                          </p>
                        )}
                      </>
                    ) : (
                      <div className="flex w-full flex-col items-center gap-3">
                        ...
                        <div className="flex w-full gap-2">
                          <Button
                            className="flex-1"
                            size="sm"
                            onClick={() => copyAddr(scanResult ?? undefined)}
                          >
                            {copied ? (
  ...
  return (
    <>
      ...
      {/* Sheet rendered via portal — lives at document.body, not inside header */}
      {sheet}
    </>
  )
}
